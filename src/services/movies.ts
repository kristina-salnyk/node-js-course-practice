...
export const removeMovie = async (movieId: string): Promise<void> => {
  const movies = await readMovies();
  const filteredMovies = movies.filter((item: Movie): boolean => item.id !== movieId);
...
export const addMovie = async (data: MovieInput): Promise<Movie> => {
  ...
  const movie: Movie = {id: generateUniqueId(), title, description, genre, duration};
  movies.push(movie);
...
export const updateMovie = async (movieId: string, data: MovieInput): Promise<Movie | undefined> => {
  const {title, description, genre, duration} = data;
  const movies = await readMovies();
